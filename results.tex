...
This claim is supported by the plots in Figure~\ref{fig:sammon}. This figure shows a Sammon mapping~\cite{sammon1969nonlinear} of the high- and low-fidelity evaluations made by all three algorithms on the three- and eight-dimensional instances of the $Griewank_2$ function from test suite $B$. The purpose of Sammon mapping is to project high dimensional data onto the two-dimensional plane, so it can be visualized, while maintaining similar ratios of euclidean distances between all the points. 
\begin{figure*}[h!]
  \centering
