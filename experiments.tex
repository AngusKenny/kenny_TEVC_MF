...
\subsection{$MO^2TOS$ algorithm}

The second method used for comparison with \AlgName{} is the $MO^2TOS$ algorithm described in Section~\ref{sec:back}. No source code for this algorithm was available online, so an implementation based on~\cite{xu2016mo2tos} was made, the pseudocode for which is given in Algorithm~\ref{alg:mo2tos}.

\begin{algorithm}[h!] 
\caption{$MO^2TOS$ procedure}
\label{alg:mo2tos}
\algsetup{linenosize=\footnotesize}
{\footnotesize
\begin{algorithmic}[1]
