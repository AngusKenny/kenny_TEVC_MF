...
Tables~\ref{tab:funcs} and~\ref{tab:errfuncs} below give the equations for the test functions and error functions mentioned in Section~\ref{sec:exp}, respectively.
\begin{table}[h!]
\centering
\caption{\small{}Test functions used for experiments in this paper. Given are the instance name, the function, the number of decision variables ($D$), the bounds ($B$) and the reference the test function is taken from.}\label{tab:funcs}
...
\midrule
$Griewank$    & $f_H(\V{x}) = \displaystyle\sum^D_{i = 1} \frac{x_i^2}{4000} - \prod^D_{i = 1}\cos{\left(\frac{x_i}{\sqrt{i}}\right)}+1$ & $\{3,5,8\}$ & $[-5,5]^D$ &\cite{griewank1981generalized}\\
\midrule  
$Michalewicz$ & $f_H(\V{x}) = \displaystyle\sum^D_{i = 1} \sin{\left(x_i\right)}\sin^{2m}{\left(\frac{ix_i^2}{\pi}\right)},\quad m = 10$ & $\{3,5,8\}$ & $[0,\pi]^D$&\cite{michalewicz2013genetic}\\
%
\bottomrule
\end{tabular}
}
